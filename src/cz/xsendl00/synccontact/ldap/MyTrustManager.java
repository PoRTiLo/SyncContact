...
import static com.unboundid.util.StaticUtils.toHex;
import static com.unboundid.util.StaticUtils.toLowerCase;
import static cz.xsendl00.synccontact.ldap.SSLMessages.ERR_CERTIFICATE_REJECTED_BY_USER;
import static cz.xsendl00.synccontact.ldap.SSLMessages.INFO_PROMPT_CLIENT_HEADING;
import static cz.xsendl00.synccontact.ldap.SSLMessages.INFO_PROMPT_ISSUER_SUBJECT;
import static cz.xsendl00.synccontact.ldap.SSLMessages.INFO_PROMPT_MD5_FINGERPRINT;
import static cz.xsendl00.synccontact.ldap.SSLMessages.INFO_PROMPT_SERVER_HEADING;
import static cz.xsendl00.synccontact.ldap.SSLMessages.INFO_PROMPT_SHA1_FINGERPRINT;
import static cz.xsendl00.synccontact.ldap.SSLMessages.INFO_PROMPT_SUBJECT;
import static cz.xsendl00.synccontact.ldap.SSLMessages.INFO_PROMPT_VALIDITY;
import static cz.xsendl00.synccontact.ldap.SSLMessages.WARN_PROMPT_EXPIRED;
import static cz.xsendl00.synccontact.ldap.SSLMessages.WARN_PROMPT_NOT_YET_VALID;
import static cz.xsendl00.synccontact.ldap.SSLMessages.WARN_PROMPT_SELF_SIGNED;


/**
 * This class provides an SSL trust manager that will interactively prompt the user to determine
 * whether to trust any certificate that is presented to it. It provides the ability to cache
 * information about certificates that had been previously trusted so that the user is not prompted
 * about the same certificate repeatedly, and it can be configured to store trusted certificates in
 * a file so that the trust information can be persisted.
 */
public final class MyTrustManager implements X509TrustManager {
  ...
  /**
   * Creates a new instance of this prompt trust manager. It will cache trust information in memory
   * but not on disk.
   */
  ...
  /**
   * Creates a new instance of this prompt trust manager. It may optionally cache trust information
   * on disk, and may also be configured to examine or ignore validity dates.
   *
   * @param acceptedCertsFile The path to a file in which the certificates that have been previously
   * accepted will be cached. It may be {@code null} if the cache should only be maintained in
   * memory.
   * @param examineValidityDates Indicates whether to reject certificates if the current time is
   * outside the validity window for the certificate.
   */
  public MyTrustManager(final String acceptedCertsFile,
  ...
  /**
   * Indicates whether this trust manager would interactively prompt the user about whether to trust
   * the provided certificate chain.
   *
   * @param chain The chain of certificates for which to make the determination.
   * @return {@code true} if this trust manger would interactively prompt the user about whether to
   * trust the certificate chain, or {@code false} if not (e.g., because the certificate is already
   * known to be trusted).
   */
  ...
  private String createCertMessage(final X509Certificate[] chain,
      final boolean serverCert,
      final String validityWarning) throws CertificateException {
    ...
    builder.append('\t'
        + INFO_PROMPT_SUBJECT.get(c.getSubjectX500Principal().getName(X500Principal.CANONICAL))
        + "\n");
    ...
    for (int i = 1; i < chain.length; i++) {
      builder.append('\t'
          + INFO_PROMPT_ISSUER_SUBJECT.get(i,
              chain[i].getSubjectX500Principal().getName(X500Principal.CANONICAL)) + "\n");
    ...
    builder.append(INFO_PROMPT_VALIDITY.get(String.valueOf(c.getNotBefore()),
   ...
   *
   * @param chain The chain of certificates for which to make the determination.
   * @param serverCert Indicates whether the certificate was presented as a server certificate or as
   * a client certificate.
   * @throws CertificateException If the provided certificate chain should not be trusted.
   */
  private synchronized void checkCertificateChain(final X509Certificate[] chain,
      final boolean serverCert) throws CertificateException {
  ...
  /**
   * Computes the fingerprint for the provided certificate using the given digest.
   *
   ...
   * @throws CertificateException If a problem is encountered while generating the certificate
   * fingerprint.
   */
  private static String getFingerprint(final X509Certificate c, final MessageDigest d)
  ...
  /**
   * Indicate whether to prompt about certificates contained in the cache if the current time is
   * outside the validity window for the certificate.
   *
   * @return {@code true} if the certificate validity time should be examined for cached
   * certificates and the user should be prompted if they are expired or not yet valid, or
   * {@code false} if cached certificates should be accepted even outside of the validity window.
   */
   ...
   *
   * @param chain The client certificate chain for which to make the determination.
   * @param authType The authentication type based on the client certificate.
   * @throws CertificateException If the provided client certificate chain should not be trusted.
   */
  @Override
   ...
   *
   * @param chain The server certificate chain for which to make the determination.
   * @param authType The key exchange algorithm used.
   * @throws CertificateException If the provided server certificate chain should not be trusted.
   */
  @Override
  ...
  /**
   * Retrieves the accepted issuer certificates for this trust manager. This will always return an
   * empty array.
   *
